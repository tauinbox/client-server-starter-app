...
- **grafana** — grafana/grafana:13.0.1, accessible at port 3001; provisioned datasource (Prometheus) and the **Nexus** dashboard (HTTP traffic, per-route p95 latency, auth events, SSE connections, Node.js runtime, and an RBAC & Reliability section: permission denials, process RSS, token-reuse alarm, uptime, queue/handle health). See [`server/README.md` → "Observability"](server/README.md#observability) for the full metric list, Prometheus alert recipes for `rbac_permission_denied_total`, and an RBAC drill-down dashboard (`doc/grafana/rbac.json`)
