...
- **Real-time notifications via SSE** — `GET /api/v1/notifications/stream` (JWT-protected) pushes three event types: `session_invalidated` (force-logout on admin password change or user delete), `permissions_updated` (silent permissions re-fetch when a user's roles change or when a role they hold has its permission set changed — fanned out to every connected holder of that role), `user_crud_events` (admin user list auto-refresh on create/update/delete/restore). Client uses `HttpClient` with `observe: 'events'` so the existing JWT interceptor attaches the Bearer token; `NotificationsService` connects on login and disconnects on logout with exponential-backoff reconnect
...
- **Plan change with proration (API)** — `POST /api/v1/billing/subscription/change` switches tier and/or billing mode instantly: Paddle computes the proration itself (`subscriptions.update`, prorated immediately), YooKassa is settled server-side per the refund-and-recharge policy (charge the new plan's whole-day remainder first, then refund the old plan's unused remainder — two fiscal documents, both visible as invoices); `/change/preview` returns the cost split without applying (the change-plan dialog UI lands with the proration-UI milestone)
- **Payment-method update (API)** — `POST /api/v1/billing/payment-method` starts the provider-hosted card replacement for the current subscription: Paddle returns its zero-amount payment-method-change checkout, YooKassa re-binds the card via a zero-amount payment whose success webhook swaps the default saved method (the old card is demoted, not deleted; the next renewal charges the new token); `past_due` subscriptions are allowed — fixing the card is how dunning recovers (the settings-page update button lands with the proration-UI milestone)
