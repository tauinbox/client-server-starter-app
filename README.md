...
|------|------|-------|--------|
| Server unit tests | Jest | `*.spec.ts` alongside source | 1747 tests passing |
| Server E2E tests | Jest | Separate config in `test/` | 251 tests; a run with Postgres + Redis + Mailpit reports 228 passing and 23 skipped (suites gated on `DB_HOST` / `REDIS_URL` skip entirely on a bare run) |
