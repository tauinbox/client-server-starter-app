...
- **Operator-safe `custom` conditions** — the `custom` branch runs `validateMongoQueryKeys()` on parsed user-supplied JSON before any merge, the same allow-list `PermissionConditionDto` applies on write: a `__proto__`/`constructor`/`prototype` key, or any `$`-operator outside `ALLOWED_MONGO_OPERATORS` (`$where`, `$regex`, `$exists`, a typo), vetoes the entire permission. Screening the runtime layer with the write layer's allow-list keeps a stored row from meaning one thing to CASL and another to the SQL list-filter translator, which cannot reproduce those operators
...
|------|------|-------|--------|
| Server unit tests | Jest | `*.spec.ts` alongside source | 1696 tests passing |
...
| Mock server | Express | `mock-server/` directory, provides full API simulation with RBAC support; parity specs in `src/__tests__/` assert its responses match the server's | 233 tests passing |
