...
- **Billing console** — "Billing" tab at `/admin/billing` (requires `manage:Billing`, hidden behind the public `billing` flag). Read-only tables of every customer's subscriptions and invoices (desktop table / handset cards) with the two M1 mutations: cancel a subscription (end-of-period or immediate, via a confirm dialog) and refund a paid invoice. Both lists are server-paginated: each table carries its own `mat-paginator` and the page opens with one bounded request per list rather than the whole table. Fully internationalized (EN / RU)
...
- Pagination response envelope: `{ data: User[], meta: { page, limit, total, totalPages } }` — the same envelope and the same `PaginationQueryDto` (`page`, `limit` capped at 100, `sortBy`, `sortOrder`) back the three billing list endpoints (`GET /billing/invoices`, `GET /admin/billing/invoices`, `GET /admin/billing/subscriptions`)
...
|------|------|-------|--------|
| Server unit tests | Jest | `*.spec.ts` alongside source | 1868 tests passing |
...
| Client unit tests | Vitest | `*.spec.ts` alongside source, runner options in `client/vitest-base.config.mjs` | 1021 tests passing |
| Client E2E tests | Playwright | `e2e/` directory, uses mock-server (4 parallel workers) | 209 tests passing |
| Mock server | Express | `mock-server/` directory, provides full API simulation with RBAC support; parity specs in `src/__tests__/` assert its responses match the server's | 370 tests passing |
