...
│       │   └── billing/
│       │       └── billing-admin-list/        # BillingAdminListComponent — read-only Subscriptions + Invoices tables (mat-table desktop / card-list handset), each server-paginated behind its own mat-paginator, per-row cancel (period-end/immediate menu + confirm) and refund (confirm); shared status-chip mixin with billing settings
...
│       └── store/          # RolesStore (route-level), ResourcesStore (route-level: resources, actions, loading), FeatureFlagsAdminStore (route-level: signalStore + withEntities<FeatureFlagResponse>), BillingAdminStore (route-level: subscriptions, invoices, per-list page state, cancel, refund)
...
│       │   ├── billing-settings/  # BillingSettingsComponent — current plan + status chip, change-plan dialog (hidden for past_due / pending cancellation), cancel (confirm dialog), credits wallet card, payment method + update action (provider-hosted redirect), usage meter (usage-mode subs), paginated invoices (table desktop / cards handset)
...
│       ├── services/       # BillingService (HTTP → /api/v1/billing/*: plans, products, subscription, invoices, payment-method (GET/POST), usage, credits, checkout, purchase, subscription/change (+ /preview), subscription/cancel, region); CheckoutRedirectService — sole owner of hosted-checkout navigation, follows a session URL only when https or same-origin (javascript:/data:/cross-origin http blocked with a translated error)
│       ├── store/          # BillingStore — route-level signalStore (plans, products, subscription, invoices + invoicesPage, paymentMethod, usage, credits, region); loadPricing/loadSettings/checkout/purchase/refreshInvoices/loadInvoicesPage/changePlan/startPaymentMethodUpdate/cancel/setRegion
...
└── shared/
    ...
    │                       # TranslocoPaginatorIntl — MatPaginatorIntl bound to the common.paginator.* keys (Material ships
    │                       # hardcoded English labels), re-emitting changes on a language switch; provided in app.config
    └── utils/              # css.utils, dialog.utils (DialogSize enum + dialogSizeConfig()),
                            ...
                            # pagination.utils (PageRequest + pageParams() HttpParams builder, PAGE_SIZE_OPTIONS),
