...
│   │   ├── guards/         # authGuard, guestGuard, permissionGuard(action, subject), instancePermissionGuard(action, subject, instanceFactory) — all funnel through ensureAuthenticated(), which redirects straight to /login when no user was ever persisted instead of firing a refresh that can only 401
│   │   ├── utils/          # ensureAuthenticated (guard funnel), isSafeReturnUrl (post-login redirects must be app-internal paths), isSameOriginUrl, navigateToLogin, token-refresh predicates
