...
| Format check | `npm run format:check` — covers `src/`, `e2e/`, `scripts/` and the root-level configs |
| Format | `npm run format` — same scope, writes fixes |
| Unit tests | `npm test` (Vitest) |
| E2E tests | `npm run test:e2e` (Playwright) |
| E2E tests (UI) | `npm run test:e2e:ui` |
| Audit dependencies | `npm run audit:ci` (`npm audit --audit-level=high --omit=dev` via `scripts/audit-ci.mjs`, same gate CI enforces; retries only a failing registry endpoint, never a real finding) |
...
```
src/app/
├── core/                   # Header, sidenav, theme toggle, storage/session-storage services, error interceptor, 404 page, NotificationsService (SSE), LayoutService (CDK Breakpoints → signals: isHandset/isTablet/isWeb), RoleCatalogService (HTTP → /api/v1/roles; the unpaginated role catalog every picker and filter reads, kept out of features/admin so users and admin do not depend on each other)
├── features/
...
│   │   ├── guards/         # featureFlagGuard(key, redirectTo = '/forbidden') — pipes through ensureAuthenticated() so an expired token gets refreshed before the flag check; returns the redirectTo UrlTree on miss
...
│   │   ├── services/       # UserService (HTTP → /api/v1/users), UserRoleService (assign/remove a role on one user → /roles/assign/:userId[/:roleId])
│   │   ├── components/
...
│       │       ├── feature-flag-form-dialog/ # FeatureFlagFormDialogComponent — top form (key, description, environments chip-grid (option-only), enabled, public) + embedded rules editor; uses Wide dialog size on desktop, .app-dialog-fullscreen-mobile panelClass on handset
│       │       └── feature-flag-rule-row/    # FeatureFlagRuleRowComponent — single rule editor, payload editor per type (user chip+autocomplete via UserService.searchCursor with 250 ms debounce, role chip+autocomplete from RoleCatalogService.getAll(), percentage discrete slider with 5% step + static value label, attribute field+op+value+customKey with chips for op=in and mat-datepicker for op=before/after), include/exclude effect surfaced via [data-effect] colored left-border + tinted background on exclude, .vertical class on handset
│       │   └── billing/
│       │       └── billing-admin-list/        # BillingAdminListComponent — read-only Subscriptions + Invoices tables (mat-table desktop / card-list handset), each cursor-paginated behind its own store + infinite-scroll sentinel, per-row cancel (period-end/immediate menu + confirm) and refund (confirm); shared status-chip mixin with billing settings
│       ├── services/       # RoleService (HTTP → /api/v1/roles; role administration only — reading the catalog is core RoleCatalogService), RbacAdminService (HTTP → /api/v1/rbac/*), FeatureFlagsAdminService (HTTP → /api/v1/admin/feature-flags/*; If-Match version on PATCH), BillingAdminService (HTTP → /api/v1/admin/billing/*: subscriptions, invoices, cancel, refund)
│       └── store/          # RolesStore (route-level), ResourcesStore (route-level: resources, actions, loading), FeatureFlagsAdminStore (route-level: signalStore + withEntities<FeatureFlagResponse>), BillingSubscriptionsStore + BillingInvoicesStore (route-level, one per list: withEntities + withCursorList, plus cancel / refund)
...
│       │   ├── billing-settings/  # BillingSettingsComponent — current plan + status chip, change-plan dialog (hidden for past_due / pending cancellation), cancel (confirm dialog — a metered plan gets its own copy warning that the closing period is charged), credits wallet card, payment method + update action (provider-hosted redirect), usage meter (usage-mode subs), cursor-paginated invoices (table desktop / cards handset, infinite scroll)
...
│       ├── directives/     # HasEntitlementDirective — *nxsHasEntitlement="'reports'"; mirrors HasFeatureDirective's surface on the entitlement axis (optional nxsHasEntitlementElse TemplateRef for an upgrade prompt); triggers the store's lazy load itself and fails closed while unloaded; advisory only — the server's EntitlementGuard is the boundary
│       ├── services/       # BillingService (HTTP → /api/v1/billing/*: plans, products, subscription, invoices, payment-method (GET/POST), usage, credits, entitlements, checkout, purchase, subscription/change (+ /preview), subscription/cancel, region); CheckoutRedirectService — sole owner of hosted-checkout navigation, follows a session URL only when https or same-origin (javascript:/data:/cross-origin http blocked with a translated error)
│       ├── store/          # EntitlementsStore — root signalStore mirroring GET /billing/entitlements ({ planKey, capabilities, limits, loaded }); load() is lazy (triggered by the first consumer, not at bootstrap — the read needs auth and most sessions never gate on it), reload() on the entitlements SSE push, clear() on login and both logout branches, has(capability)/limit(key) as per-key computed signals, limit() keyed by the shared EntitlementLimitKey union rather than a free-form string. Its first limit consumer is billing settings, which renders the plan's concurrent-device allowance as limit('sessions') ?? MAX_CONCURRENT_SESSIONS — from this mirror, not from PlanResponse.limits, which cannot express the Free fallback. BillingStore — route-level signalStore (plans, products, subscription, paymentMethod, usage, credits, region + the invoice collection via withCursorList); loadPricing/loadSettings/checkout/purchase/refreshInvoices/loadMoreInvoices/changePlan/startPaymentMethodUpdate/cancel/setRegion
...
└── shared/
    ...
    ├── store/              # withCursorList<T>({ fallbackKey }) — the shared cursor-list feature every list store composes:
    ...
    └── utils/              # css.utils, dialog.utils (DialogSize enum + dialogSizeConfig()),
                            ...
                            # pagination.utils (CursorPageRequest + cursorParams() HttpParams builder, CURSOR_PAGE_SIZE),
...
- **AuthStore** (`providedIn: 'root'`) — pure state container. State: `accessToken` (in-memory signal only, never persisted), `user` (persisted to `localStorage` as `auth_user` key for page-reload detection; read back through the `isPersistedUser` guard in `store/storage-guards.ts`, so a stale or malformed entry is ignored and the app starts logged out rather than rendering a partial user), `ability: AppAbility | null`. Computed: `isAuthenticated` (access token present), `user`, `roles`. Methods: `hasPermissions(check)` (one check or an array; an **empty array denies** - `[].every()` would otherwise grant, and the ability signal is read before that bail so callers reacting to it stay subscribed), `setRules(rules)`, `hasPersistedUser()`, `saveAuthResponse()`, `clearSession()`. No `HttpClient` dependency. RBAC checks always go through `hasPermissions` — never compare role names against the `'admin'` string (use `SYSTEM_ROLES.ADMIN` from `@app/shared/constants` for the rare display-only label)
- **CaptchaService** (`providedIn: 'root'`) — fetches `/api/v1/auth/captcha-config` once per session (cached in a signal), lazily injects the Cloudflare Turnstile script (`https://challenges.cloudflare.com/turnstile/v0/api.js?render=explicit`), exposes `loadConfig()` and `loadScript()` consumed by `CaptchaWidgetComponent`. **Neither call caches a failure.** A failed config fetch rejects and clears the in-flight promise, so the next widget mount retries: caching an `enabled: false` fallback instead would latch the captcha off for the whole tab, and the server would then keep answering `CAPTCHA_REQUIRED` for a widget that can no longer obtain a site key. A failed script load likewise drops the cached promise, and the dead `<script>` tag is removed from the DOM: a tag that already fired `error` never fires another event, so a retry has to append a fresh one. `CaptchaWidgetComponent` renders `status: 'error'` on either rejection
- **AuthService** (`providedIn: 'root'`) — HTTP operations (login/register/logout/refresh/profile/OAuth accounts/`fetchPermissions(): Promise<void>`/`fetchRbacMetadata(): Promise<void>`). `register(data, captchaToken?)` and `forgotPassword(email, captchaToken?)` accept an optional Turnstile token included in the body when provided. `login()` uses `switchMap` to await `fetchPermissions()` before emitting — ensures permissions are loaded before route guards evaluate. `refreshTokens()` POSTs `{}` — the `refresh_token` HttpOnly cookie is sent automatically by the browser. The request itself cannot be aborted (it is owned by the promise inside the Web Locks callback), so `TokenService.cancelRefresh()` bumps a session epoch instead: a response that arrives after a logout, a forced logout or a `session_invalidated` push resolves to `null` and never writes the session back. `logout()` refreshes first when the in-memory access token has expired - the state a tab that slept through its token's lifetime wakes up in - because `POST /auth/logout` is JWT-guarded and excluded from the interceptor's refresh-and-retry, so posting it with a stale token answers 401 and leaves the refresh token live on the server behind a logged-out UI; a refresh that fails means the session is already invalidated and falls through to the local teardown. Session teardown is a single routine: `logout()` runs it on both branches, and `TokenService.forceLogout()` reaches it through the `sessionCleared$` subject (`TokenService` cannot import `AuthService` — that would close a dependency cycle), so every exit path disconnects the SSE stream and clears the session, the cached RBAC metadata, the feature flags and the entitlements. The exits that never reach `logout()` - `ensureAuthenticated` and `guestGuard` after a failed refresh, and the `catch` in `provideAppInitializer` - run the same routine through the public `clearSession()` delegate rather than calling `AuthStore.clearSession()`, which would clear only the token and the persisted user. A logout in one tab ends the session in the others too: the constructor listens for the `storage` event, which fires in every other same-origin tab and never in the one that made the change, and hands a removal of `auth_user` to `TokenService.forceLogout(router.url)` - full teardown, no HTTP call (the other tab already revoked the session server-side), landing on `/login?returnUrl=`. Without it the access token, which lives in memory per tab, kept every other tab working for its full remaining lifetime. The listener is deliberately narrow: it keys on that one key transitioning to `null`, in `localStorage`, so a login or a profile save in another tab (both of which *write* the key) leaves this tab alone, as does a change to any other persisted key; and a tab holding no session at all is left where it is rather than pulled to `/login`. `provideAppInitializer` awaits `fetchPermissions().then(() => fetchRbacMetadata())` for authenticated users, or attempts a cookie-refresh first when `hasPersistedUser()` is true (page reload with no in-memory token). A failed `fetchPermissions()` resolves rather than rejects - callers chain `fetchRbacMetadata()` on it - but it is no longer silent: the failure is reported through `NotifyService.error(err, 'errors.general.permissionsUnavailable')`, because the ability stays `null` and every `permissionGuard` then redirects to `/forbidden`, which is indistinguishable from a genuine denial. The fallback key is a root key, not a `features/auth` scoped one: `fetchPermissions()` also runs from the app initializer and from `features/admin`, where that lazily loaded scope is absent. `fetchRbacMetadata()` is permission-gated: `GET /rbac/metadata` requires `permissions:read` on the server, so the method resolves without a request when `hasPermissions({ action: 'read', subject: 'Permission' })` is false — callers must load permissions first. When permitted it implements stale-while-revalidate: returns immediately if data is cached in `RbacMetadataStore`, refreshes in background
- **UsersStore** (route-level at `/users`) — entity-based store with `withEntities<User>()`. Unified state: `filters: UserSearch` (empty = all users, filled = search via `GET /users/search/cursor`), single `load()`/`loadMore()` pair with **infinite scroll** (page size 20; `upsertEntities` appends; `hasMore` computed signal drives sentinel visibility; `isLoadingMore` shows spinner). `setFilters()` and `setSorting()` update state; component calls `load()` after each change. The list's search field is capped at 255 characters (`MAX_USER_FILTER_LENGTH`) — the search endpoint answers a longer filter with a 400, so the form blocks the submit instead
- **RbacMetadataStore** (`providedIn: 'root'`) — NgRx Signal Store with stale-while-revalidate localStorage caching for resources/actions metadata; the cached payload is read back through the `isCachedRbacMetadata` guard in `store/storage-guards.ts` (elements included, since `subjectMap` reads `name` / `subject` off every resource), and a payload that fails it is discarded so the catalog simply refetches. Loaded at bootstrap via `AuthService.fetchRbacMetadata()` (only for users holding `permissions:read`; the server gates the endpoint the same way). Cleared on logout via `clear()` so the previous user's role/permission structure does not persist on shared devices. Computed: `subjectMap` (resource name to CASL subject)
- **ThemeService** — `theme` signal (`'light'` | `'dark'`), system preference detection, persists to localStorage; a persisted value outside those two modes is ignored rather than applied to `data-theme`
- **LocalStorageService / SessionStorageService** — thin wrappers over a shared `web-storage.ts` helper that owns the parse/serialize rules. Strings are persisted unquoted so plain values stay readable outside Angular (`main.ts` reads the language preference before bootstrap). `getItem` accepts an optional type guard; values that fail it return `null` instead of being cast
...
- **NotificationsService** (`providedIn: 'root'`) — SSE client using `HttpClient` with `observe: 'events'` so the JWT interceptor attaches `Authorization: Bearer` automatically. Parses `HttpDownloadProgressEvent.partialText` with offset tracking. Exposes: `sessionInvalidated$` (calls `tokenService.forceLogout()`), `permissionsUpdated$` (triggers `authService.fetchPermissions()` followed by the permission-gated `fetchRbacMetadata()`), `userCrudEvents$` (drives user list refresh; the server sends these only to clients whose abilities allow `users:search`). `connect()` called after login and session restore; `disconnect()` called on every session teardown, explicit logout and forced logout alike. Exponential backoff retry (3 s → 60 s cap, up to 10 retries, `resetOnSuccess`); auto-reconnects after retry exhaustion or server-initiated close if still authenticated. Server sends 30 s heartbeat (empty `data:` frames) to prevent proxy idle timeout. Because Angular's HTTP backends retain the full response body for the lifetime of a request (every raw chunk plus a cumulative `partialText`), a never-ending stream would grow without bound, so the connection is recycled every 4–8 h (per-client jitter, so clients never reconnect in lockstep) — `disconnect()` + `connect()`, which drops those buffers; the recycle is skipped while disconnected or unauthenticated
- **NotifyService** (`providedIn: 'root'`) — centralised `MatSnackBar` helper. Methods: `success(messageKey, params?)`, `info(messageKey, params?)`, `warn(messageKey, params?)`, `error(messageKey, params?)`, `error(httpError, fallbackKey?)`. Translates the message via `TranslocoService`, uses a translated `common.close` action, and relies on `MAT_SNACK_BAR_DEFAULT_OPTIONS` for duration/position so call sites don't repeat config. The `HttpErrorResponse` overload mirrors the `errorInterceptor` parsing chain: prefer translated `errorKey`, then server `message`, then translated `fallbackKey`, else status code. **A server `errorKey` is never rendered outside this funnel** - it is only a translation key when a translation for it exists, so translating it directly puts a raw dot-path on screen the first time the server ships a key ahead of the client i18n file (it happened three times in `features/auth`). A `no-restricted-syntax` selector in `eslint.config.mjs` bans `translate(errorKey)` in any shape and bans passing one as a `NotifyService` message key; `http-error.utils.ts` carries the single `eslint-disable-next-line` for the funnel itself, which is allowed because it compares the result against the key and falls through when nothing translated it. Client-owned fallback keys are named `fallbackKey`, never `errorKey`, so the word means exactly one thing
...
2. **jwtInterceptor** — attaches `Authorization: Bearer` header to same-origin requests only (relative URLs resolve against the app origin; absolute cross-origin URLs never receive the token and their 401s skip the refresh path), handles 401 with token refresh + request retry, uses `shareReplay(1)` to prevent concurrent refreshes
...
- **Runner config**: `vitest-base.config.mjs`, referenced from the `test` target's `runnerConfig` option — it carries the Vitest options the builder does not expose. It sets `testTimeout: 15000`, because a TestBed spec pays the component's JIT compile cost on its first test: the slowest specs here measure 2.9-3.6 s on an idle machine and 4.0-6.2 s once the workers compete for CPU, so the 5000 ms Vitest default (Vitest itself uses 15000 in browser mode) failed whichever slow files lost the CPU race and passed them on the next run. `src/vitest-runner-config.spec.ts` asserts the raised value so the wiring cannot silently regress
...
- Modular fixture architecture in `e2e/fixtures/`:
  ...
  - `helpers.ts` — `loginViaUi()`, `loginViaUiKeepSse()` (variant that skips `networkidle` so tests can hold a real SSE stream open), `expectAuthRedirect()`, `expectForbiddenRedirect()`, `openedDialog()`, `routeApiToMockServer()` (the `/api` redirect + SSE stub `base.fixture` applies to the fixture page; Playwright routes are per page, so a test that opens a second tab applies it to that tab itself)
- **Dialogs must be awaited with `openedDialog(page)`, not `expect(getByRole('dialog')).toBeVisible()`**, before any keyboard interaction. A dialog turns visible ~150 ms before it is safe to type into: `MatDialog` defers its focus trap to the end of the open animation and then focuses the first tabbable element, while Playwright's `fill()` is two round trips (in-page `select() + focus()`, then a separate `Input.insertText` aimed at whatever holds focus by then). A trap firing between the two appends the text to the first field and leaves the intended one empty and marked required. Measured on the Add Action dialog: 3 of 98 fills corrupted without the wait, 0 of 98 with it
- Test structure: organized by module in `e2e/auth/`, `e2e/users/`, and `e2e/admin/`
- **Accessibility**: `e2e/a11y.spec.ts` runs `@axe-core/playwright` (WCAG 2.1 AA) against every major route; `e2e/keyboard-nav.spec.ts` verifies keyboard-only flows (login, sidenav, user-edit, dialog focus trap). The file sets its own `timeout: 60_000` via `test.describe.configure`: an axe scan is CPU-bound and competes with the other three workers, so the same scan that takes ~9 s with a single worker takes 21-30 s in a full parallel run — the heaviest page (the users list) repeatedly grazed the 30 s default
- **Live RBAC + auth regression net**: refresh-token reuse detection (`refresh-token-reuse.spec.ts`), SSE-driven role revocation hides admin link (`role-revocation-via-sse.spec.ts`), admin-on-/admin auto-redirect to /forbidden (`admin/admin-panel-permission-loss.spec.ts`), reactive 401 → refresh → retry (`reactive-token-refresh.spec.ts`), logout + browser Back navigation (`logout-back-button.spec.ts`), a logout in one tab ending the session in a second tab of the same context (`cross-tab-logout.spec.ts` - the observing tab parks on `/profile`, because a list page keeps issuing cursor requests and the first 401 would reach `/login` through the jwt interceptor with or without the listener), OAuth unlink-last-provider safety (`oauth-unlink-last-provider.spec.ts`), wire-contract assertion that `auth_user.roles` is `RoleResponse[]` (`post-login-admin-badge.spec.ts`), OAuth sign-in completing the session so a guarded return URL activates instead of bouncing to /forbidden (`oauth-callback-session.spec.ts`), SSE-driven feature-flag toggle propagation (`admin/feature-flags.spec.ts`)
...
- Coverage: 219 Playwright tests across auth/users/admin/billing/a11y/keyboard/visual suites; 1144 Vitest unit tests covering login, register, profile (incl. self-service email change, which shares one submit with the name and password edits), session-restore, cross-tab logout, lockout, email verification, password reset (with password confirmation), users list/detail/edit/search (including admin email-change confirmation dialog and the soft-delete/restore flow), admin roles/resources/feature-flags management, effective-permissions preview, admin-panel auto-redirect when permissions are revoked mid-session, a11y audit, keyboard navigation. Error translation tests verify `errorKey` → Transloco pipeline for login, register, and global interceptor snackbar.
