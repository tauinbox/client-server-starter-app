...
- Modular fixture architecture in `e2e/fixtures/`:
  ...
  - `helpers.ts` — `loginViaUi()`, `loginViaUiKeepSse()` (variant that skips `networkidle` so tests can hold a real SSE stream open), `expectAuthRedirect()`, `expectForbiddenRedirect()`, `openedDialog()`
