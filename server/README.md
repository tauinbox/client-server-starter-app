...
- **CaslAbilityFactory** — builds `AppAbility` from user roles + permissions; used by `AuthController` to return CASL packed rules via `packRules()` from `GET /permissions`. Partitions rules allow-first / deny-last so permissions with `conditions.effect === 'deny'` register as CASL `cannot()` rules and reliably override prior allows for the same `(resource, action)` pair. Conditions are fail-closed: when `resolveConditions()` vetoes a permission (denied MongoQuery operator, or restriction branches that resolve to an empty query — empty `fieldMatch` arrays, unknown `userAttr` attribute, `custom` parsing to `{}`/invalid JSON), an `allow` registers nothing and a `deny` registers as an unconditional `cannot()`; only a branch-less condition (bare `effect`) registers unconditionally
