...
│   ├── services/           # AuthService, OAuthService, TokenGeneratorService, RefreshTokenService, SessionLimitService (maxSessionsFor(userId) = EntitlementService.limitFor(userId, 'sessions') ?? MAX_CONCURRENT_SESSIONS — the concurrent-session allowance is a plan dimension; both AuthService.login and OAuthService.loginWithOAuth pass its result to pruneOldestTokens, so a paid allowance is not silently trimmed by whichever path the user signs in on. Fails open: a resolver rejection is caught and degrades to the constant, because a billing outage must never become a login outage), OAuthAccountService, TokenCleanupService, ResourceService, ActionService, ResourceSyncService
...
│   ├── entitlements/       # EntitlementService.capabilitiesFor(userId) (active/trialing/past_due-in-grace → plan.entitlements, else Free, unioned with active CustomerGrants — non-revoked, non-expired — from paid one-time sku purchases), per-user cache keyed by the shared CacheVersionCounter (common/utils/cache-version-counter.ts — atomic Redis INCR, in-memory read-modify-write fallback); `limitFor(userId, key)` reads the already-resolved `limits` off the same cache (null = the plan carries no limit under that key, caller applies its own default); @RequireEntitlement('<cap>') decorator + EntitlementGuard (403). `entitlements.module.ts` — **EntitlementsModule**, an auth-free module holding the service + guard and importing only `TypeOrmModule.forFeature([Customer, CustomerGrant, Plan, Subscription])`. It exists so `AuthModule` can read an entitlement without closing a cycle with `BillingModule → AuthModule`: the resolver never depended on auth, only its file location suggested it did. `BillingModule` re-exports the module, so existing consumers import `BillingModule` unchanged. `EntitlementChangedListener` deliberately stays in `BillingModule` — it binds billing-owned events and pushes through NotificationsService
